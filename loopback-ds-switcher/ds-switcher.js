...
module.exports = function (RED) {
  ...
  var inbound = function (msg) {
    saveRec(msg.model, msg.payload, function (err, result) {
      if (err) {
        console.log('could not save returned rec ' + err);
      }
  ...
  function removeOldOBefores(Model, id) {
    ...
    var remote = modelRemotes ? (modelRemotes[type] ? modelRemotes[type]._listeners : null) : null;
    if (remote) {
  ...
  function AsyncObserverNode(config) {
    ...
    if (Model !== undefined) {
      ...
      globalContext.set('sendEx', outbound);
      // this is now available to other nodes
      globalContext.set('sendInternal', inbound);

      removeOldObservers(Model, node.id);
      removeOldOBefores(Model, node.id);

      Model.observe(method, new DsSwitcherObserver(node, modelName, method, altds, legacyUrl).observe);
...
var DsSwitcherObserver = function (node, modelName, methodName, altds, legUrl) {
  var _node = node;
  var _altds = altds;
  var _legUrl = legUrl;
